...
def plotTimeLines(forecast, reference, title):
  ...
  ax1 = plt.plot(forecast['times'], forecast['observations'], "black", markersize=8, label="Observations")
  ax1 = plt.legend(prop={'size': 10}, loc=2)
  ax1 = plt.title(title)
  ax1 = plt.xlabel("Dates")
...
for ID in IDs:
  ...
  plotScatter(fullOutput["data"][ID], "Simulations %s, R= %.02d, AC = %.02d, KGE = %.02d" %(str(config.get('Main options', 'RunName')), output[0][ID,3], output[0][ID,4], output[0][ID,5]))
  plotScatter(fullOutput2["data"][ID], str(config.get('Reference options', 'RunName')))
